...
import com.roamprocess1.roaming4world.widgets.RegistrationNotification;
...
public class SipNotifications {
	...
	String imagefile = "R4WIMGTOCONTACTCHATSEND@@";
	...
	public SipNotifications(Context aContext) {
		...
		System.out.println("SipNotifications class");
		if (!isInit) {
			cancelAll();
			cancelCalls();
			isInit = true;
	...
	private String formatNotificationTitle(int title, long accId) {
        ...
        if ((acc != null) && !TextUtils.isEmpty(acc.display_name)) {
        	...
        	System.out.println("SipNotifications account display   "+acc.display_name);
            notifTitle.append(" - ");
            notifTitle.append("Roaming4world");
        }
		...
		if (!CustomDistribution.supportMessaging()) {
		 ...
		 if (!msg.getFrom().equalsIgnoreCase(viewingRemoteFrom)) {
			...
			if (messageNotification == null) {
				...
				messageNotification.setSmallIcon(SipUri.isPhoneNumber(msNumber) ? R.drawable.r4wlogowhitefill : R.drawable.r4wlogowhitefill);
			...
			if(!msg.getFrom().equals("sip:registrar@kamailio.org") && !msg.getBody().contains("[Offline message -")){
