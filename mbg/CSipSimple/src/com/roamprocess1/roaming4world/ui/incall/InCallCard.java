...
package com.roamprocess1.roaming4world.ui.incall;
...
import com.roamprocess1.roaming4world.api.SipCallSession.MediaState;
...
public class InCallCard extends FrameLayout implements OnClickListener, Callback {
    ...
    private boolean r4woutCall=true;
    RelativeLayout call_card_container;
    ...
    private void initControllerView() {
        ...
        call_card_container = (RelativeLayout) findViewById(R.id.call_card_container);
    ...
    private void updateMenuView() {
        ...
        if(!added) {
            ...
            /*
             * Code added by Varun top set height of in call controls -- START
             */
            //layoutParams.height= 240;
            /*
             * Code added by Varun top set height of in call controls -- END
             */
    ...
    private final Runnable postLayout = new Runnable() {
        ...
        public void run() {
            ...
            if(w > 0 && h > 0) {
                ...
                /*
                 * Code Added by Varun(R4W) - to decrease height of end call button -- START
                 */
                lp.height = 110;
                /*
                 * Code Added by Varun(R4W) - to decrease height of end call button -- END
                 */
    ...
    private void updateQuickActions() {
        ...
        btnMenuBuilder.findItem(R.id.holdCallButton).setVisible(active).setTitle(callInfo.isLocalHeld() ? R.string.resume_call : R.string.hold_call);
        
        btnMenuBuilder.findItem(R.id.videoCallButton).setVisible(active && canVideo && !callInfo.mediaHasVideo());
        if(r4woutCall==false){
        	 btnMenuBuilder.findItem(R.id.videoCallButton).setVisible(false);
        	 }else{
        	 btnMenuBuilder.findItem(R.id.videoCallButton).setVisible(active && canVideo && !callInfo.mediaHasVideo());
        }
        ...
        if (aRemoteUri != null && !aRemoteUri.equalsIgnoreCase(cachedRemoteUri)) {
            ...
            if (callInfo.getAccId() != SipProfile.INVALID_ID) {
                ...
                if (acc != null && acc.display_name != null) {
                	...
                	//display name
                	// statusTextBuffer.append("Display Name" + acc.display_name + " : ");
                   System.out.println("calling numbner:"+acc.display_name );
                	statusTextBuffer.append("" + acc.display_name + " : ");
                }
            } else {
            	
            	//display name add prefix
            	
                statusTextBuffer.append("");
            }

            statusTextBuffer.append(uriInfos.userName);
            
            String sipaddress=statusTextBuffer.toString();
            System.out.println("incallcard:"+sipaddress);
         
            
            String remoteaddress[]=sipaddress.split(":");
            String number=remoteaddress[1].trim();
            System.out.println("incallcard Sip:"+number);
           
            
            if(number.startsWith("011")){
            	
            	r4woutCall=false;
            	  System.out.println("incallcard Sip: inside if");
            	  number = number.substring(3);
            	 remoteSipAddress.setText("Roaming4World :+"+number);
            }else{
            	
            	 System.out.println("incallcard Sip: inside else");
            	   remoteSipAddress.setText("Roaming4World :+"+number);
            }
            
            System.out.println("incallcard remoteSipAddress"+statusTextBuffer.toString());
    ...
    private static class ContactLoadedHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.arg1 == LOAD_CALLER_INFO) {
                ...
                System.out.println("ContactLoadedHandler");
               
                System.out.println("lci.callerInfo" + lci.callerInfo.phoneNumber);
                
                String num = lci.callerInfo.phoneNumber.toString();
                if(num.startsWith("+")){
                	num = num.substring(1);
                }
                num = stripNumber(num);
                ...
                String path = "/sdcard/R4W/ProfilePic/" + num +".png";
                System.out.println("msg adp out - path="+path);
                File imageFile = new File(path);
        		if(imageFile.exists()){
        			try {
            			lci.target.photo.setImageURI(Uri.parse(path));
    	                lci.target.call_card_container.setBackgroundColor(Color.BLACK);
    	               } catch (Exception e) {
