...
public class CallLogAdapter extends GroupingListAdapter
    ...
    private void setPhoto(CallLogListItemViews views, CallerInfo ci, String nu) {

        views.quickContactView.assignContactUri(ci.contactContentUri);        
        System.out.println("setPhoto "+nu);
        if(nu != null)
        {
        	if(nu.startsWith("+")){
        		nu = nu.substring(1);
        ...
        nu = stripNumber(nu);
        System.out.println("ci.number=" + nu);
		...
		if(imageFile.exists()){
			try {
    			...
    			views.quickContactView.getImageView().setImageBitmap(bm);
    		} catch (Exception e) {
				// TODO: handle exception
				try {
					views.quickContactView.getImageView().setImageURI(Uri.parse(path));
