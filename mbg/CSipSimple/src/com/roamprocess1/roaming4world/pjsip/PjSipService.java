...
package com.roamprocess1.roaming4world.pjsip;
...
import com.roamprocess1.roaming4world.api.SipUri.ParsedSipContactInfos;
import com.roamprocess1.roaming4world.pjsip.earlylock.EarlyLockModule;
import com.roamprocess1.roaming4world.pjsip.player.IPlayerHandler;
import com.roamprocess1.roaming4world.pjsip.player.impl.SimpleWavPlayerHandler;
import com.roamprocess1.roaming4world.pjsip.recorder.IRecorderHandler;
import com.roamprocess1.roaming4world.pjsip.recorder.impl.SimpleWavRecorderHandler;
import com.roamprocess1.roaming4world.pjsip.reghandler.RegHandlerModule;
import com.roamprocess1.roaming4world.pjsip.sipclf.SipClfModule;
import com.roamprocess1.roaming4world.service.MediaManager;
import com.roamprocess1.roaming4world.service.SipService;
import com.roamprocess1.roaming4world.service.SipService.SameThreadException;
import com.roamprocess1.roaming4world.service.SipService.SipRunnable;
import com.roamprocess1.roaming4world.service.SipService.ToCall;
import com.roamprocess1.roaming4world.utils.ExtraPlugins;
import com.roamprocess1.roaming4world.utils.ExtraPlugins.DynCodecInfos;
import com.roamprocess1.roaming4world.utils.CallLogHelper;
...
import com.roamprocess1.roaming4world.utils.TimerWrapper;
import com.roamprocess1.roaming4world.utils.video.VideoUtilsWrapper;
import com.roamprocess1.roaming4world.utils.video.VideoUtilsWrapper.VideoCaptureCapability;
import com.roamprocess1.roaming4world.utils.video.VideoUtilsWrapper.VideoCaptureDeviceInfo;
...
//                status = pjsua.acc_modify(accId[0], nCfg);
        ...
        synchronized (codecs) {
            if (!codecs_initialized) {
                ...
                for (i = 0; i < nbrCodecs; i++) {
                    ...
                    System.out.println("videocodecID-" + codecId);
                    //* add by nitin
                    if(i==1)
                    {
                ...
                if (ni != null) {
                    ...
                    synchronized (codecs) {
                        ...
                        for (String codec : video_codecs) {
                            ...
                            System.out.println("VideoCodec--"+codec+"-");
    ...
    public int makeCall(String callee, int accountId, Bundle b) throws SameThreadException {
    	System.out.println("pjService.makeCall(callee, accountId, options) called");
        if (!created) {
            return -1;
        }
    	System.out.println("pjService.makeCall(callee, accountId, options) after created");
        
        
        final ToCall toCall = sanitizeSipUri(callee, accountId);
        if (toCall != null) {
        	System.out.println("if (toCall != null) pjService.makeCall(callee, accountId, options) ");
        	...
        	System.out.println("if (b != null) before pjService.makeCall(callee, accountId, options) ");
            if (b != null) {
            	System.out.println("if (b != null) in pjService.makeCall(callee, accountId, options) ");
                Bundle extraHeaders = b.getBundle(SipCallSession.OPT_CALL_EXTRA_HEADERS);
                if (extraHeaders != null) {
                	System.out.println("if (extraHeaders != null) pjService.makeCall(callee, accountId, options) ");
                   for (String key : extraHeaders.keySet()) {
                   	System.out.println("for (String key : extraHeaders.keySet()) pjService.makeCall(callee, accountId, options) ");
                        try {
                            String value = extraHeaders.getString(key);
                            if (!TextUtils.isEmpty(value)) {
                            	System.out.println("if (!TextUtils.isEmpty(value))pjService.makeCall(callee, accountId, options) ");
        	...
        	System.out.println("pjsua.call_make_call before pjService.makeCall(callee, accountId, options) ");
    ...
    private int sendDtmf(final int callId, String keyPressed) throws SameThreadException {
        ...
        if (!TextUtils.isEmpty(remainingDtmf)) {
            ...
            if (tasksTimer == null) {
                tasksTimer = new Timer("com.roamprocess1.roaming4world.PjSipServiceTasks");
            }
    ...
    /**
     * Send typing using SIP server
     */
    public ToCall sendTyping(String callee, String message, long accountId)
            throws SameThreadException {
        if (!created) {
            return null;
        }
        ...
        if (toCall != null) {
            ...
            int status = pjsua.im_typing(toCall.getPjsipAccountId(), uri, 0, null);
            return (status == pjsuaConstants.PJ_SUCCESS) ? toCall : null;
        }
        return toCall;
    }
