...
import com.roamprocess1.roaming4world.api.SipCallSession.StatusCode;
...
import com.roamprocess1.roaming4world.service.impl.SipCallSessionImpl;
...
import com.roamprocess1.roaming4world.utils.Threading;
...
public class UAStateReceiver extends Callback {
    ...
    public void on_call_state(final int callId, pjsip_event e) {
    	System.out.println(" on_call_state(final int callId, pjsip_event e)");
    	System.out.println("pjsip_event e--" + e);
        ...
        try {
            ...
            if (callState == SipCallSession.InvState.DISCONNECTED) {
            	System.out.println(" if (callState == SipCallSession.InvState.DISCONNECTED)");
            	if (pjService.mediaManager != null) {
            	...
            	System.out.println(" if End (callState == SipCallSession.InvState.DISCONNECTED)");
            } else {
                if (ongoingCallLock != null && !ongoingCallLock.isHeld()) {
                    ongoingCallLock.acquire();
                }
    ...
    public void on_buddy_state(int buddyId) {
        ...
        System.out.println("PresenceATesting--On buddy " + buddyId + " state " + binfo.getMonitor_pres() + " state "
                + PjSipService.pjStrToString(binfo.getStatus_text()));
    ...
    public void on_pager(int callId, pj_str_t from, pj_str_t to, pj_str_t contact,
        ...
        SipMessage msg = new SipMessage(canonicFromStr, toStr, contactStr, bodyStr, mimeStr,
                date, SipMessage.MESSAGE_TYPE_INBOX, fromStr,0);
                ...
                case ON_CALL_STATE: {
                    ...
                    switch (callState) {
                        ...
                        case SipCallSession.InvState.DISCONNECTED:
                        	System.out.println("case SipCallSession.InvState.DISCONNECTED:");
                            if (stateReceiver.pjService.mediaManager != null) {
                                stateReceiver.pjService.mediaManager.stopRing();
                            }
    ...
    public void initService(PjSipService srv) {
        ...
        if (eventLock == null) {
            ...
            eventLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                    "com.roamprocess1.roaming4world.inEventLock");
        ...
        if (ongoingCallLock == null) {
            ...
            ongoingCallLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                    "com.roamprocess1.roaming4world.ongoingCallLock");
